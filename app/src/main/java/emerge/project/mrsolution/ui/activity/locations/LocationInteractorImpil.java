...
import emerge.project.mrsolution.BuildConfig;
import emerge.project.mrsolution.services.api.ApiClient;
import emerge.project.mrsolution.services.api.ApiInterface;
...
public class LocationInteractorImpil implements LocationInteractor {
    ...
    public void postLocation(Context context, final LocationEntitie locationEntitie, final int isAfterSuggestion, final OnPostLocationFinishedListener onPostLocationFinishedListener) {
        ...
        } else if (locationEntitie.getLocationTypeID() == 0) {
            onPostLocationFinishedListener.postLocationError("Please select the location type");
                        ...
                        .subscribe(new Observer<ArrayList<LocationType>>() {
                            ...
                            public void onComplete() {
                                if (locationTypeList.isEmpty()) {
                                    onGetLocationTypeListFinishedListener.locationTypeListEmpty();
                                } else {
                                    for (LocationType loc : locationTypeList) {
                                        list.add(new LocationType(loc.getId(), loc.getName()));
                                    }
                                    onGetLocationTypeListFinishedListener.locationTypeList(list);
                                }
                            }
                        });
            } catch (Exception ex) {
    ...
    @Override
    public void getSelectedLocationType(LocationType locationType, OnSelectedLocationTypeFinishedListener onSelectedLocationTypeFinishedListener) {
        onSelectedLocationTypeFinishedListener.selectedLocationType(locationType);
    }
}
